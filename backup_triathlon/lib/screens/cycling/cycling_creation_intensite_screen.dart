...
import '../../services/cycling_service.dart';
import '../../widgets/cycling_exercice_form.dart';

class CyclingCreationIntensiteScreen extends StatefulWidget {
  const CyclingCreationIntensiteScreen({super.key});

  @override
  _CyclingCreationIntensiteScreenState createState() =>
      _CyclingCreationIntensiteScreenState();
}

class _CyclingCreationIntensiteScreenState
    extends State<CyclingCreationIntensiteScreen> {
  ...
  final CyclingService _cyclingService = CyclingService();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Création Séance Cyclisme'),
              ...
              children: [
                ...
                Expanded(
                  child: ElevatedButton.icon(
                    ...
                    style: ElevatedButton.styleFrom(
                      backgroundColor: TriathlonColors.cycling.withOpacity(0.9),
                      padding: const EdgeInsets.symmetric(vertical: 15),
                    ),
  ...
  void _ajouterExercice() {
    TriathlonExercice nouvelExercice = TriathlonExercice(
      ...
      distance: 1000, // 1km par défaut
      nbSeries: 3,
      nbRepetitions: 1,
      valeurReference: 250, // FTP par défaut (250 watts)
  ...
  Future<void> _sauvegarderSeance() async {
    ...
    if (nomSeance.isEmpty) {
      nomSeance =
          'Séance cyclisme ${DateTime.now().day}/${DateTime.now().month}';
    }
    ...
    bool saved = await _cyclingService.saveSeance(seance);
